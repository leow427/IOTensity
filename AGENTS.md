...
One scene unit is one metre; origin is the floor below the monitor centre. X increases right (−3..3), Y up (0.15..3), Z into the room (−0.7..4). All inputs share the same clamp. Location changes X/Z only; Height changes Y only. Camera transforms are not room coordinates. Orbs/cards share `resolveColor`: selected-editor calibration overrides ordinary output even at zero brightness; Location uses X-only green → orange, Height cyan → violet. Selecting or positioning a physical light temporarily sends that same calibration color to its ESP32, including before Save Room. This native preview expires two seconds after the last selection/position change and clears on deselect, unbind, delete, save/discard, navigation, blur, Stop or shutdown. Only the selected device is overridden; it resumes saved Sync output or turns off afterward. Preview requests never change saved bindings or configuration, and use no frontend transmission timer. Keep TypeScript/Rust calibration aligned with `tests/fixtures/placement-colors.json`. Identify remains available for blinking a device during discovery.
